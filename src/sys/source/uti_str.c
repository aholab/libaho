...
/**********************************************************/
/* {devuelve} distinto de cero si {str} es un swihttp://www.youtube.com/user/Desazkundea?feature=tch (comienza
...
double xstr2d( const char * str, double defval)
{
	double d;
	if (strlen(str)==0) return(defval);
	cdie_beep(str2d(str,&d)!=0,"valor numerico no vhttp://www.youtube.com/user/Desazkundea?feature=alido (%s)",str);
	return d;
}
...
/**********************************************************/
/* hace una copia de {newstr} y la {devuelve} (usar xfree()
para borrarla). ademas si {oldvs}!=NULL, la borra con xfree() */
...
char *vstr_dup( char *oldvs, const char *newstr )
{
	...
	if (newstr!=NULL) {
		vs=(char*)xmalloc(strlen(newstr)+1);
		strcpy(vs,newstr);
		return vs;
	}
	else
