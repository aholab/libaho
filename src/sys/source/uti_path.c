...
{
	char * tmp;
	tmp=xstrdup(&(path[path_namepos(path)]));
	strcpy(path,new_dir);
	strcat(path,tmp);
	xfree(tmp);
	return(path);
...
{
	char * ex;
	ex=xstrdup(&(path[path_extpos(path)]));
...
int frename_ext_del( const char * name, const char * new_ext )
{
	int retval;
	char * tmp;
	tmp=(char *)xmalloc(strlen(name)+strlen(new_ext)+1);
	strcpy(tmp,name);
...
fichero destino, (se hace xmalloc(), por lo que el usuario debe hacer
xfree() de esta cadena).
...
#define STRNCAT(dest,source,n) { \
	...
	char * tmp = (char *)xmalloc(LEN(source)+LEN(dest)+LEN(ddir)+LEN(dext)+4);
...
#define STRNCAT(dest,source,n) { \
  dest[strlen(dest)+1]=0; \
	...
	char * tmp = (char *)xmalloc(LEN(source)+LEN(dest)+LEN(dpath)+4);
