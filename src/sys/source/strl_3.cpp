...
/*</DOC>*/
{
	...
	sw = (char**)xmalloc(sizeof(char*)*(nsw+1));
	for (i=0, p=fmt.first(); i<nsw; i++, p=fmt.next(p))
		sw[i] = xstrdup((const char *)(fmt.itemkey(p)));
	sw[nsw]=NULL;

	_clargs_case_sensitive=1;
	df=NULL;
	if (opts.contains("DefCfgFile")) df = xstrdup(opts.val("DefCfgFile"));  // fich. config.
	else if (! opts.bval("NoDefCfgFile",FALSE)) { 
		//fprintf(stderr,"entra\n");
//		fprintf(stderr,"argv[0]=%s\tpat_namepos=%d\tsrc=%s\n", argv[0], path_namepos(argv[0]), argv[0]+path_namepos(argv[0]) );
